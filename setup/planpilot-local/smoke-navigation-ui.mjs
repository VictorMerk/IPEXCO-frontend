...
const planPilotUrl = (
  process.env.PLANPILOT_URL ?? "http://127.0.0.1:5000"
).replace(/\/+$/, "");
const screenshotPath = process.env.IPEXCO_NAVIGATION_SCREENSHOT_PATH;
...
if (
  ...
  throw new Error("The smoke-test viewport is invalid.");
}
const port = Number(
  process.env.IPEXCO_CHROME_DEBUG_PORT ?? 12000 + (process.pid % 30000),
);
...
try {
    ...
    source: `(() => {
      ...
      XMLHttpRequest.prototype.open = function(method, url, ...rest) {
        ...
        this.addEventListener('load', () => {
          if (this.__navigationSmokeRequest?.method !== 'POST') return;
          try {
            const response = JSON.parse(this.responseText);
            if (/\\/api\\/planpilot\\/sessions\\/?$/.test(this.__navigationSmokeRequest.url)) {
              window.__navigationSmokeRunId = response.runId ?? null;
            }
            if (Array.isArray(response.facets)) {
              window.__navigationSmokeReadOnlyFacetIds = response.facets
                .filter((facet) => facet.selectable === false)
                .map((facet) => facet.id);
            }
          } catch {}
        }, { once: true });
  ...
  const promotedHorizon = await waitForExpression(
    cdp,
    `(() => {
    if (document.querySelector('.status--error') || document.querySelector('.status')?.textContent?.includes('Loading')) return null;
    const summary = document.querySelector('.session-summary')?.textContent?.replace(/\\s+/g, ' ').trim();
    const plans = document.querySelector('.solutions__summary')?.textContent?.replace(/\\s+/g, ' ').trim();
    return summary?.includes('horizon 8')
      && summary.includes('Horizon 5 was below the shortest plan')
      && plans === 'Showing 1 plans'
      ? { summary, plans }
      : null;
  })()`,
    180_000,
  ...
  const initialPlan = await waitForExpression(
    cdp,
    `(() => {
    ...
    const session = document.querySelector('.session-summary')?.textContent?.replace(/\\s+/g, ' ').trim();
  ...
  await evaluate(
    cdp,
    `([...document.querySelectorAll('.analysis-controls button')]
    .find((button) => button.textContent?.includes('Count plans'))).click()`,
  );
  const counted = await waitForExpression(
    cdp,
    `(() => {
    if (document.querySelector('.status--error') || document.querySelector('.recalculating')) return null;
    const summary = document.querySelector('.solutions__summary')?.textContent?.replace(/\\s+/g, ' ').trim();
  ...
  const stagedLabel = await evaluate(
    cdp,
    `(() => {
    const group = [...document.querySelectorAll('.facet-group')]
      .find((item) => item.querySelector('.facet-group__title')?.textContent?.includes('State'));
  ...
  const committed = await waitForExpression(
    cdp,
    `(() => {
    if (document.querySelector('.status--error') || document.querySelector('.recalculating')) return null;
  ...
  await evaluate(
    ...
    `(() => {
      const group = [...document.querySelectorAll('.facet-group')]
        .find((item) => item.querySelector('.facet-group__title')?.textContent?.includes('Action'));
      const header = group?.querySelector('mat-expansion-panel-header');
      if (!group?.querySelector('.facet')) header?.click();
      return true;
    })()`,
  );
  await waitForExpression(
    cdp,
    `[...document.querySelectorAll('.facet-group')].some((group) =>
      group.querySelector('.facet-group__title')?.textContent?.includes('Action')
      && group.querySelector('.facet'))`,
    10_000,
  );
  const enforcedAction = await evaluate(
    cdp,
    `(() => {
      const group = [...document.querySelectorAll('.facet-group')]
        .find((item) => item.querySelector('.facet-group__title')?.textContent?.includes('Action'));
  ...
  await waitForExpression(
    cdp,
    `document.querySelectorAll('.facet--decision').length >= 2
      && document.querySelector('.submit-bar__info')?.textContent?.replace(/\\s+/g, ' ').trim() === 'No pending changes'`,
    90_000,
  );
  const readOnlyFacetsHidden = await waitForExpression(
    cdp,
    `(() => {
    const readOnly = window.__navigationSmokeReadOnlyFacetIds;
    if (!Array.isArray(readOnly) || readOnly.length === 0) return null;
    const open = [...document.querySelectorAll('.column:first-child .facet')]
      .map((facet) => facet.dataset.facetId);
    return readOnly.every((facetId) => !open.includes(facetId))
      ? { readOnly: readOnly.length, open: open.length }
      : null;
  })()`,
    10_000,
  );
  ...
  if (cdp) {
    try {
      const runId = await evaluate(
        cdp,
        "window.__navigationSmokeRunId ?? null",
      );
      if (runId) {
        await fetch(
